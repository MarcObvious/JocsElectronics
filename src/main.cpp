...
SDL_Window* createWindow(const char* caption, int width, int height,
	...
	//create the window
	SDL_Window *window = SDL_CreateWindow(caption, 100, 100, width, height,
...
void mainLoop() {
	...
	while (1) {
		...
		while (SDL_PollEvent(&sdlEvent)) {
			...
			case SDL_WINDOWEVENT:
				...
				case SDL_WINDOWEVENT_RESIZED: //resize opengl context
					game->setWindowSize(sdlEvent.window.data1,
		...
		game->mouse_delta.set(game->mouse_position.x - x,
				game->mouse_position.y - y);
		game->mouse_position.set(x, y);
