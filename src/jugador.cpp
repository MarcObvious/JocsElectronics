...
bool Jugador::canvia_a_vius() {
	bool surt = false;
	for (unsigned int i = _apunta; i < _aliats.size() && !surt; i++) {
		if (_aliats.at(i)->getVida() > 0){

		}
	}


	for(unsigned int i = 0; i< _apunta; i++ ) {
		if ( World::getInstance()->_naus_aliades[i]->getVida() < 0 );
			return false;
	}
		canvia_control();	return false;
}
void Jugador::update(double seconds_elapsed, const Uint8* keystate) {
	_espera = _espera + seconds_elapsed;
	double speed = seconds_elapsed * 100;

	if (_controlat->getVida() < 0) {
		canvia_control();
	}
	...
	if (keystate[SDL_SCANCODE_SPACE]) {
		...
		BulletManager::getInstance()->createBullet(_controlat->getPosition(),
				_controlat->getPosition(),
