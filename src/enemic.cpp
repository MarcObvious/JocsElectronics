...
Enemic::Enemic(Nau* contr, Entity* waypoint) : Controller(contr){
	//_name<<" i es enemic";
	_controlat = contr;
	_objectiu = waypoint; //Els objectius podrien no venir de mon i podrien variar.
...
bool Enemic::distancia_objectiu(){
	if(_controlat->getPosition().distance(_objectiu->getPosition()) > 1000)
			return false;
	return true;
}
...
void Enemic::actua(double seconds_elapsed) {
	...
	if (!distancia_objectiu()) {
		//retorna al objectiu
	}
	else {
		for (unsigned int i; i < _objectius_aliats.size(); i++) {
			if (_controlat->getPosition().distance(_objectius_aliats.at(i)->getPosition()) < 1000){
				std::cout << "CABRON\n";
			}
		}
	}
