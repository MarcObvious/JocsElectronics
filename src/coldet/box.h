...
public:
	...
	Box(float x, float y, float z, float sx, float sy, float sz) :
			m_Pos(x, y, z), m_Size(sx, sy, sz), m_Center(x + 0.5f * sx, y + 0.5f * sy, z + 0.5f * sz) {
	}
	...
	/** Copy constructor */
	Box(const Box& b) :
			m_Pos(b.m_Pos), m_Size(b.m_Size), m_Center(b.m_Center) {
	}
	virtual ~Box() {
	}
...
public:
	BoxTreeInnerNode(const Vector3D& pos, const Vector3D& size, int logdepth) :
			BoxTreeNode(pos, size), m_First(0), m_Second(0), m_logdepth(logdepth), m_OwnFirst(true), m_OwnSecond(true) {
	}
	~BoxTreeInnerNode() {
		if (m_OwnFirst)
			delete m_First;
